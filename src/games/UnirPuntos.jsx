...
import { STYLES, Orb, Orbs, colorGrad, BG, PANEL, BackButton, P1, P2, sanitizePlayers } from "./shared.jsx";
          ...
          <div className={`flex flex-col items-center w-full lg:w-auto ${tab!=="board"?"hidden sm:flex":"flex"}`}>
            ...
            <div className="p-1.5 sm:p-2.5 rounded-2xl sm:rounded-3xl" style={{background:"linear-gradient(145deg,rgba(20,24,48,0.95),rgba(10,14,26,0.98))",border:"1px solid rgba(255,255,255,0.06)",boxShadow:"0 25px 60px rgba(0,0,0,0.5)"}}>
              <div className="flex mb-0.5 px-0.5" style={{gap:`${GAP}px`}}>
                {Array.from({length:COLS}).map((_,c)=>{const show=hoverCol===c&&isMyTurn&&!gs.winner&&!gs.isDraw&&getDropRow(gs.board,c)>=0;return(<div key={c} className="flex items-center justify-center cursor-pointer" style={{width:`${CELL}px`,height:`${Math.round(CELL*0.5)}px`}} onMouseEnter={()=>setHoverCol(c)} onMouseLeave={()=>setHoverCol(-1)} onClick={()=>handleDrop(c)}>{show&&<div className="rounded-full" style={{width:`${Math.round(CELL*0.38)}px`,height:`${Math.round(CELL*0.38)}px`,background:myColor?colorGrad(myColor):"#555",opacity:0.5,animation:"bob 1.2s ease-in-out infinite"}}/>}</div>);})}
              </div>
              <div className="rounded-xl sm:rounded-2xl p-0.5 sm:p-1" style={{background:"linear-gradient(180deg,rgba(15,23,42,0.4),rgba(20,24,48,0.5))",border:"1px solid rgba(255,255,255,0.03)"}}>
                {gs.board.map((row,r)=>(<div key={r} className="flex" style={{gap:`${GAP}px`,marginBottom:r<ROWS-1?`${GAP}px`:0}}>{row.map((cell,c)=>(<div key={c} className="relative rounded-full transition-colors duration-200" style={{width:`${CELL}px`,height:`${CELL}px`,background:"radial-gradient(circle,rgba(10,14,26,0.95),rgba(10,14,26,0.75))",boxShadow:"inset 0 2px 6px rgba(0,0,0,0.5)",cursor:isMyTurn&&!gs.winner&&!gs.isDraw?"pointer":"default"}} onMouseEnter={()=>setHoverCol(c)} onMouseLeave={()=>setHoverCol(-1)} onClick={()=>handleDrop(c)}>{cell!==EMPTY&&<Piece color={gs.players[cell]?.color||"green"} isWinning={isWC(r,c)} isLast={gs.lastMove?.row===r&&gs.lastMove?.col===c} isNew={animCell?.row===r&&animCell?.col===c}/>}{hoverCol===c&&isMyTurn&&getDropRow(gs.board,c)===r&&cell===EMPTY&&!gs.winner&&!gs.isDraw&&!animating&&<div className="absolute inset-1 rounded-full" style={{background:myColor?colorGrad(myColor):"#555",opacity:0.1}}/>}</div>))}</div>))}
              </div>
            </div>
